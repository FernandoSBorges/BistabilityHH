...
#define nrn_init _nrn_init__RegnStim
#define _nrn_initial _nrn_initial__RegnStim
#define nrn_cur _nrn_cur__RegnStim
#define _nrn_current _nrn_current__RegnStim
#define nrn_jacob _nrn_jacob__RegnStim
#define nrn_state _nrn_state__RegnStim
#define _net_receive _net_receive__RegnStim 
#define init_sequence init_sequence__RegnStim 
#define next_invl next_invl__RegnStim 
#define noiseFromRandom noiseFromRandom__RegnStim 
#define seed seed__RegnStim 
...
#define erand erand_RegnStim
#define invl invl_RegnStim
 extern double erand( );
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 RegnStim /home/fernando/Dropbox/Biestabilidade-HH/BistabilityHH/info/CA1-NetPyNE-model-master/x86_64/regn_stim.mod\n");
...
double invl (  double _lmean ) {
 ...
 if ( _lmean <= 0. ) {
   ...
   if ( noise  == 0.0 ) {
     ...
     _linvl = tspike + _lmean + noise * _lmean * erand ( _threadargs_ ) - t ;
     if ( _linvl <= 0.0 ) {
       _linvl = .01 ;
       }
     }
   tspike = tspike + _lmean ;
...
{
 {
   ...
   if ( start >= 0.0  && number > 0.0 ) {
     on = 1.0 ;
     event = start + noise * interval * erand ( _threadargs_ ) ;
     if ( event < 0.0 ) {
       event = 0.0 ;
       }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/fernando/Dropbox/Biestabilidade-HH/BistabilityHH/info/CA1-NetPyNE-model-master/mod/regn_stim.mod";
static const char* nmodl_file_text = 
